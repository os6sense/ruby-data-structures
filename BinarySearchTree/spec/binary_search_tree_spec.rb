  ...
  describe :Node do
    ...
    describe :initialize do
      it "takes an integer as a key" do
        Node.new(1).should be_instance_of(Node)
      end

      it "takes a character as a key" do
        Node.new('a').should be_instance_of(Node)
      end

      it "takes a string as a key" do
        Node.new("a string").should be_instance_of(Node)
      end

      it "takes a symbol as a key" do
        Node.new(:fred).should be_instance_of(Node)
      end

      it "takes an arbitrary object as a key" do
        Node.new(Object.new).should be_instance_of(Node)
      end
    end
    ...
    describe :right do
      ...
      it "returns the right node" do
        root.insert(right = Node.new(100))
        root.right.should eq right
      end

      it "can be assigned to a new value" do
        root.insert(Node.new(100))
        root.right = Node.new(1000)
    ...
    describe :insert do
      ...
      context "when the node is empty" do
        context "and when the key is less than the value of the node" do
          it "becomes the left branch" do
            root.insert(left = Node.new(1))
            root.left.should eq left
          end
        end
        context "and when the key is greater than the value of the node" do
    ...
    describe :parent do
      ...
      it "is nil for the root node" do
    ...
    describe :find do
      ...
      before(:each) do
        [1, 2, 3, 4, 5, 21, 1213, 3, 13, 4412].each do | x |
          root.insert(Node.new(x))
        end
      end

      it "returns nil if the node is not present" do
        root.find(Node.new(123)).should eq nil
      end

      it "it returns the node if the node is present" do
        found = root.find(n) 
        found.should eq n
      end
    end
...
shared_examples "binary_search_tree" do
  describe BinarySearchTree do
    ...
    let(:simple_tree) { BinarySearchTree.new() }
